 ...
 class wpad_frontend_comment_form{
 	...
 	function __construct(){
		...
		*/
		...
		/*
		* Add anchor to comment list
		*/

		add_filter( 'wpad_before_comment_list' , array( $this , 'wpad_add_custom_anchor' ) );

 	}

 	function wpad_add_custom_anchor(){

 		echo '<div id="comments" class="wpad_comments_anchor"></div>';
 		echo '<div id="respond" class="wpad_comments_anchor"></div>';
	...
	** Show like or dislike button on the comment list
 	*/
 	...
 	function get_custom_wrapper_css( $css ){

 		$split_css = explode( ' ' , $css );

 		foreach ( $split_css as $key => $value ) {
 			if( !empty( $value ) ){
	 			echo $value . '_group ';
	 		}
	 	}
	...
	** Show user name text field
 	*/
 	...
 	function get_user_name_field( $elements , $user_name = null , $disabled = null ){ 
 		...
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_user_name">
     		<label><?php _e( $label , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<input <?php echo $required . ' ' . $disabled; ?> class="input_control <?php echo $elements['css_name']; ?>" type="text" name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" value="<?php echo $user_name; ?>" size="<?php echo $elements['size']; ?>" key="<?php echo $elements['meta_key']; ?>" element="user_name">
       			</div>
	...
	*/
 	...
 	function get_email_required_field_input( $elements , $user_email = null , $disabled = null ){ 
 		...
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_user_email">
     		<label><?php _e( $label , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<input <?php echo $disabled; ?> class="<?php echo $required; ?> input_control <?php echo $elements['css_name']; ?>" type="text" name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" value="<?php echo $user_email; ?>" size="<?php echo $elements['size']; ?>" laxEmail="true" key="<?php echo $elements['meta_key']; ?>" element="email">
	...
	*/
 	...
 	function get_section_break( $elements ){ 
 		...
 		if( !empty($elements['label']) ){
			echo '<h4>' . __( $elements['label'] , 'wpad' ) . '</h4>';
		}
 		if( !empty($elements['help_text']) ){ ?>
			
			<p><?php _e( $elements['help_text'] , 'wpad' ); ?></p>
	...
	*/
	...
	function get_user_image_field( $elements ){
		...
		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_user_image_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
	...
	*/
 	...
 	function get_text_field( $elements ){ 
 		...
 		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_text_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<input <?php echo $required; ?> class="input_control <?php echo $elements['css_name']; ?>" type="text" name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" value="<?php echo $elements['default_value']; ?>" size="<?php echo $elements['size']; ?>" key="<?php echo $elements['meta_key']; ?>" element="text">
       			</div>
       			<div class="wpad_error" style="display:none"></div>
       			<?php echo $this->help_text( $elements['help_text'] ); ?>
     		</div>
	...
	*/
 	...
 	function get_comment_field( $elements ){ 
 		...
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_comment">
     		<label><?php _e( $label , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<textarea <?php echo $required; ?> class="input_control <?php echo $elements['css_name']; ?>"  name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" value="<?php echo $elements['default_value']; ?>" rows="<?php echo $elements['rows']; ?>" cols="<?php echo $elements['column']; ?>" key="<?php echo $elements['meta_key']; ?>" element="comment"></textarea>
       			</div>
       			<div class="wpad_error" style="display:none"></div>
       			<?php echo $this->help_text( $elements['help_text'] ); ?>
     		</div>
	...
	*/
 	...
 	function get_textarea_field( $elements ){ 
 		...
 		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_textarea_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<textarea <?php echo $required; ?> class="input_control <?php echo $elements['css_name']; ?>"  name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" rows="<?php echo $elements['rows']; ?>" cols="<?php echo $elements['column']; ?>" key="<?php echo $elements['meta_key']; ?>" ><?php echo $elements['default_value']; ?></textarea>
	...
	*/
 	...
 	function get_radio_field( $elements ){
 		...
 		if( !empty($elements['options']) ): ?>
	 		...
	 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_radio_element">
	     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
	     			...
	     			<div class="wpad_input_wrap">
	 	    			...
	 	    			$key = wp_generate_password( 25, false, false );
		 	    			...
		 	    			foreach( $elements['options'] as $value ){ ?>
		 	    				<label>
		 	      					<input <?php echo $required; ?> class="input_control <?php echo $elements['css_name']; ?>" type="radio" name="<?php echo $elements['input_name']; ?>" value="<?php echo $value['option']; ?>" key="<?php echo $elements['meta_key']; ?>" ><?php _e( $value['label'] , 'wpad' ); ?>
		 	      				</label>
		 	      				<?php 
		 	      			} 
	...
	*/
 	...
 	function get_checkbox_field( $elements ){
 		...
 		if( !empty( $elements['options'] ) ): ?>
	 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_checkbox_element">
	     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
	     		<div class="input_container wpad_radio_wrapper">
	     			<div class="wpad_input_wrap">
	 	    			<?php 
	 	    			...
	 	    			foreach( $elements['options'] as $value ){ ?>
	 	    				<label>
	 	      					<input <?php echo $required; ?> class="input_control <?php echo $elements['css_name']; ?>" type="checkbox" name="<?php echo $elements['input_name']; ?>" value="<?php echo $value['option']; ?>" key="<?php echo $elements['meta_key']; ?>"><?php _e( $value['label'] , 'wpad' ); ?>
	 	      				</label>
	 	      				<?php 
	 	      			}  ?> 	      			
	 	      		</div>
	...
	*/
 	...
 	function get_select_field( $elements ){
 		...
 		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_select_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container wpad_radio_wrapper">
     			<div class="wpad_input_wrap">
 	    			<?php 
 	    			...
 	    			if( $elements['options'] ): ?>
 	    				<select name="<?php echo $elements['input_name']; ?>" class="<?php echo $required; ?> <?php echo $elements['css_name']; ?>" key="<?php echo $elements['meta_key']; ?>" >
 			    			...
 			    			if( $elements['select_first_option'] != '' ){
 		    					echo '<option value="" selected="selected">' . __( $elements['select_first_option'] , 'wpad' ) . '</option>';
 		    				} 
 			    			foreach( $elements['options'] as $value ): ?>
 			      				<option value="<?php echo $value['option']; ?>"><?php _e( $value['label'] , 'wpad' ); ?></option>
 			      				<?php 
 			      			endforeach; ?> 
 		      			</select>
 		      		<?php 
	...
	*/
 	...
 	function get_email_field( $elements ){ 
 		...
 		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_email_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<input class="<?php echo $required; ?> input_control <?php echo $elements['css_name']; ?>" type="text" name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" value="<?php echo $elements['default_value']; ?>" size="<?php echo $elements['size']; ?>" laxEmail="true" key="<?php echo $elements['meta_key']; ?>" >
	...
	*/
 	...
 	function get_url_field( $elements ){ 
 		...
 		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_url_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container">
     			<div class="wpad_input_wrap">
       				<input class="<?php echo $required; ?> input_control <?php echo $elements['css_name']; ?>" type="text" name="<?php echo $elements['input_name']; ?>" placeholder="<?php _e( $elements['placeholder_text'] , 'wpad' ); ?>" value="<?php echo $elements['default_value']; ?>" size="<?php echo $elements['size']; ?>" url="true" key="<?php echo $elements['meta_key']; ?>">
       			</div>
       			<div class="wpad_error" style="display:none"></div>
       			<?php echo $this->help_text( $elements['help_text'] ); ?>
     		</div>
	...
	*/
 	...
 	function get_multi_select_field( $elements ){
 		...
 		$required = ( $elements['required'] == 'yes' ? 'required' : '' ); 
 		$asterisk = ( $elements['required'] == 'yes' ? '<span class="wpad_required"> *</span>' : '' ); ?>
 		<div class="wpad_form_group <?php $this->get_custom_wrapper_css( $elements['css_name'] ); ?>" ids="wpad_multi_select_element">
     		<label><?php _e( $elements['label'] , 'wpad' ); echo $asterisk; ?></label>
     		<div class="input_container wpad_radio_wrapper">
     			<div class="wpad_input_wrap">
 	    			<?php 
 	    			...
 	    			if( $elements['options'] ): ?>
 	    				<select name="<?php echo $elements['input_name']; ?>" class="<?php echo $required; ?> <?php echo $elements['css_name']; ?>" multiple key="<?php echo $elements['meta_key']; ?>">
 			    			<?php 
 			    			if( $elements['select_first_option'] != '' ){
 		    					echo '<option value="">' . __( $elements['select_first_option'] , 'wpad' ) . '</option>';
 		    				} 
 			    			foreach( $elements['options'] as $value ): ?>
 			      				<option value="<?php echo $value['option']; ?>"><?php _e( $value['label'] , 'wpad' ); ?></option>
 			      				<?php 
	...
	*/
 	...
 	function help_text( $data ){
 		if( $data != '' ){
 			return '<p class="wpad_help_text">' . __( $data , 'wpad' ) . '</p>';
 		}
 	}
	...
	*/
 	...
 	function count_comments( $post_id , $results , $option , $id , $status = null ){ 
		...
		if( $status == 'woocommerce' ){
			...
			if( $count < 2 && $count > 0) {
 				echo '<h3>' . $count . __( " Comment on " , 'wpad' ) . $title . ' </h3>';
 			} elseif( $count == 0 ){
 				_e( 'There are no reviews yet. Be the first one to review.' , 'wpad' );
 			} else {
 				echo '<h3>' . $count  .  __( " Comments on " , 'wpad' ) . $title . ' </h3>';
 			}
 		}
	...
	*/
 	...
 	function timeAgo($time_ago){
 	    ...
 	    // Seconds
 	    if($seconds <= 60){
 	        return __( "just now" , 'wpad' );
 	    }
 	    //Minutes
 	    else if($minutes <=60){
 	        if($minutes==1){
 	            return  __( "one minute ago" , 'wpad' );
 	        }
 	        else{
 	            $wpad_min = __( "minutes ago" , 'wpad');
 	            return $minutes . ' ' . $wpad_min;
 	        }
 	    }
 	    //Hours
 	    else if($hours <=24){
 	        if($hours==1){
 	            return __( "an hour ago" , 'wpad' );
 	        }else{
 	            $wpad_hr =  __( "hrs ago" , 'wpad' );
 	            return $hours . ' ' . $wpad_hr;
 	        }
 	    }
 	    //Days
 	    else if($days <= 7){
 	        if($days==1){
 	            return __( "yesterday" , 'wpad' );
 	        }else{
 	            $wpad_days = __( "days ago" , 'wpad' );
 	            return $days . ' ' . $wpad_days;
 	        }
 	    }
 	    //Weeks
 	    else if($weeks <= 4.3){
 	        if($weeks==1){
 	            return __( "a week ago" , 'wpad' );
 	        }else{
 	            $wpad_week = __( "weeks ago" , 'wpad' );
 	            return $weeks . ' ' . $wpad_week;
 	        }
 	    }
 	    //Months
 	    else if($months <=12){
 	        if($months==1){
 	            return __( "a month ago" , 'wpad' );
 	        }else{
 	            $wpad_month = __( "months ago" , 'wpad' );
 	            return $months . ' ' . $wpad_month;
 	        }
 	    }
 	    //Years
 	    else{
 	        if($years==1){
 	            return __( "one year ago" , 'wpad' );
 	        }else{
 	            $wpad_yr = __( "years ago" , 'wpad' );
 	            return $years . ' ' . $wpad_yr;
 	        }
 	    }
 	}
 	...
 	*/
 	...
 	function show_comment_lists( $id = null , $layout = null ){
 		...
 		if( !empty( $_POST ) && defined( 'DOING_AJAX' ) && DOING_AJAX ){
 			...
 			$get_post_id = sanitize_text_field( $_POST['post_id'] );
 			$get_page_no = sanitize_text_field( $_POST['page_no'] );

 			$post = get_post( $get_post_id );
 			$id = sanitize_text_field( $_POST['form_id'] );
 			$per_page = $option[$id]['other']['pagination_per_page'];
 			$offset = ( $get_page_no - 1 ) * $per_page;
 			$page_no = sanitize_text_field( $get_page_no );
 		...
 		if( !empty($results) ){ 
 			...
 			if( empty( $_POST['post_id'] ) || empty( $_POST['form_id'] ) ){
 				return $content;	
 			} else {
 	...
 	*/
 	...
 	function paginate_links( $count_posts , $page_no , $per_page , $option ){ 
 		if($count_posts != 0){
             ...
             $showitems = ($range * 2)+1;
             $pagination = '<li><a class="not_click">' . __( 'Page ' , 'wpad' ) . $page_no . __( ' of ' , 'wpad' ) . $page . '</a></li>';
             if($page_no > 2 && ($page_no > $range_increase) && ($showitems < $page)){
             	$first = !empty($option['other']['text_for_first_page']) ? $option['other']['text_for_first_page'] : '&laquo; First';
                 $pagination .= '<li><a href="javascript:void(0)" page_no="1">' . $first . '</a></li>';
             }
             if($page_no > 1 && $showitems < $page){
             	$previous = !empty($option['other']['text_for_previous_page']) ? $option['other']['text_for_previous_page'] : '&lsaquo; Previous';
             ...
             if ($page_no < $page && $showitems < $page){
             	$next = !empty($option['other']['text_for_next_page']) ? $option['other']['text_for_next_page'] : 'Next &rsaquo;';
                 $pagination .= '<li><a href="javascript:void(0)" page_no="' . ($page_no + 1) . '">' . $next . '</a></li>';
             }
             if ($page_no < $page-1 &&  $page_no+$range-1 < $page && $showitems < $page){
             	$last = !empty($option['other']['text_for_last_page']) ? $option['other']['text_for_last_page'] : 'Last &raquo;';
 	...
 	*/
 	...
 	function get_comment_section( $value , $layout , $comment_time , $option , $id ){ ?>
 		<div class="wpad_row" id="<?php echo 'wpad_comment_' . $value['comment_ID'];?>">
 			<?php $this->default_layout( $value , $comment_time , $option , $id ); ?>
 		</div>
 		<?php
 	}
 	...
 	function report_comment( $comment_id ){ 
 		...
 		<a href="javascript:void(0)" class="wpad_comment_report" comment_id="<?php echo $comment_id; ?>"><?php _e( 'Report' , 'wpad' ); ?></a>
 	...
 	function default_layout( $value , $comment_time , $option , $id ){ ?>
 		<div class="wpad_content_comment">
 			...
 			<div class="wpad_content_wrap">
 				...
 				$this->show_verified_user_tag( $value , $option[$id]['other'] );
 				...
 				if( $option[$id]['other']['comment_position'] == 1 ){
					...
					echo '<p>' . sanitize_text_field( $value['comment_content'] ) . '</p>';
 					...
 					<div class="wpad_comment_meta">
 						<ul>
 							...
 							foreach( $data[$id] as $key => $value1 ){
 								...
 								if( !empty( $value1['meta_key'] ) && is_numeric( $key ) && $value1['meta_key'] != 'user_name' && $value1['meta_key'] != 'user_email' && $value1['custom_field'] != 'user_image' && $value1['meta_key'] != 'wpad_comment' && $privelage == true ){
	...
	** Show the comment metas
 	*/
 	...
 	function display_comment_metas_frontend( $label , $meta_value ){
 		echo '<li>';
 		echo '<label>' . __( $label , 'wpad' ) . ' : </label>
 		<span class="meta_value"> ' . sanitize_text_field( $meta_value ) . ' </span>';
 		echo '</li>';
 	}
 	...
 	*/
 	...
 	function success_message_submit( $id ){
 		...
 		if( $result == 0 ){
 			_e( "Your comment is awaiting moderation" , 'wpad' );
 		} else {
	...
	** Show the comment form to the user.
		...
		// *******************************************
 		...
 		if( $data[$id]['other']['comment_status'] != 'show_comments' ){  ?>
 			...
 			<div class="wpad_comment_fields_wrapper">
 				...
 				<p class="wpad_email_not_publish"><?php _e( "Your email address will not be published. Required fields are marked" , 'wpad' ); ?></p>
 				<?php 
		 					...
		 					<div class="input_container wpad_radio_wrapper">
		 						<div class="wpad_input_wrap">
			 						<label>
				 						...
				 						<?php _e('Email me when my comment gets approved.','wpad'); ?>
				 					</label>
				 				</div>
		 					</div>
	...
	function textarea_on_report_comment(){ 
		...
		<div>
			<h5><?php _e( 'Your Message' , 'wpad' ); ?></h5>
	...
	function repost_comment_section(){ ?>
		...
		<div id="wpad_report_comment" title="<?php _e( 'Report This Comment' , 'wpad' ); ?>" style="display:none">
	...
	/*
	** Check product page or single page
	*/

	function check_single_product_page(){
	...
	*/
 	...
 	function comment_form( $atts ){ 
		...
		* Show only in Single and Product page
 		*/
 		...
 		if( $this->check_single_product_page() == false ){
			return;
		}
 			...
 			if( $key == $id ): 
				...
				// *************************************************
 				...
 				<form id="wp_advance_comment_form" method="post" class="wpad_form_horizontal">
 					...
 					/*
					* Display Comment listing on the top of the form
 					*/
 					if( empty( $data[$id]['other']['comment_order_position'] ) || $data[$id]['other']['comment_order_position'] == 'above' ){
 						$this->wpad_comment_position( $id , $layout ); 
 					}
					...
					* If Verified Customer Reviews is checked(enable)
 					*/
 					...
 					if( function_exists( 'is_product' ) && is_product() && class_exists( 'wpad_woocommerce' ) && class_exists( 'WooCommerce' ) && !empty( $data[$id]['other']['verified_customer_review'] ) && $data[$id]['other']['verified_customer_review'] == 'enable' ){
 						...
 						if( $this->check_woo_verified_user() == true ){
 							...
 							<div class="wpad_comment_fields_wrapper not_logged_in_wrapper">
	 							...
	 							<h3><?php echo $args['reply_title']; ?></h3>
 					...
 					/*
					* Display Comment listing on the Bottom of the form
 					*/

 					if( !empty( $data[$id]['other']['comment_order_position'] ) && $data[$id]['other']['comment_order_position'] == 'below' ){
 						$this->wpad_comment_position( $id , $layout ); 
 					} ?>
 	...
 	function show_verified_user_tag( $value , $options ){

 		if( $this->check_woo_verified_user( $value ) == true && !empty( $options['verified_customer_badge'] ) && $options['verified_customer_badge'] == 'enable' ){

 			$args = array();
 			$args['verified_owner'] = __( 'Verified Owner' , 'wpad' );

			$args = apply_filters( 'wpad_verified_owner' , $args );

 			echo '<span class="wpad_verified_owner">' . $args['verified_owner'] . '</span>';
 	...
 	/**
	* @param {boolen} ( If user has purchased return true if not false )
 	*/

 	function check_woo_verified_user( $value = null ){
 		...
 		if( $value == null ){
 			...
 			$product_id = $value['comment_post_ID'];
 			$user_id = $value['user_id'];
 			$user_email = $value['comment_author_email'];
 		}
 	
 		if( function_exists( 'wc_customer_bought_product' ) && wc_customer_bought_product( $user_email , $user_id , $product_id ) ){
 	...
 	function wpad_comment_position( $id , $layout ){ 

 		/*
		* @hook wpad_before_comment_list ( Executes before comment list starts )
		*/

		do_action( 'wpad_before_comment_list' ); ?>
 		...
 		foreach( $data as $key => $value ){
 			...
 			if( (isset( $value['required'] ) && $value['required'] == 'yes') || !empty($value['error_message']) || !empty( $value['more_validation'] ) ){
 				...
 				if( (isset( $value['error_message'] ) && !empty( $value['error_message'] )) || !empty( $value['more_validation'] ) ){
 					$parameters['required'] = !empty($value['error_message']) ? __( $value['error_message'] , 'wpad' ) : __( 'This field is required' , 'wpad' );
