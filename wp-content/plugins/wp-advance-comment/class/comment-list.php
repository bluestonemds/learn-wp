...
class wpad_advance_comment{
	...
	function row_action_pending_all( $value ){ 
		...
		<span class="reply hide-if-no-js"> | 
			...
			<a href="<?php echo admin_url('admin.php?page=wpad_reply_form') . '&parentid=' . $value->comment_ID . '&formid=' . $commnet_form_id; ?>" title="<?php _e( 'Reply to this comment' , 'wpad' ); ?>" class="">
		...
		<span class="edit"> | 
			...
			<a title="<?php _e( 'Edit comment' , 'wpad' ); ?>" href="<?php echo admin_url('admin.php?page=wpad_saved_edit_form') . '&commentid=' . $value->comment_ID . '&formid=' . $commnet_form_id; ?>">
	...
	function update_comment_status(){
		...
		$status = sanitize_text_field( $_POST['comment_status'] );
	...
	function view_page( $post_id ){
		...
		echo '<a href="' . get_permalink( $post_id ) . '">' . __( 'View Page' , 'wpad' ) . '</a>';
	...
	function get_wpad_comments(){ 
		...
		$page_no = empty( $post_page ) ? '0' : $post_page;
		...
		$comment_status_post = sanitize_text_field( $_POST['comment_status'] );
		$comment_status = !empty( $comment_status_post ) ? $comment_status_post : '';
			...
			foreach ( $comments as $value ){ 
				...
				<tr class="<?php echo $status_class; ?>">
					...
					<td class="comment column-comment">
						...
						<p><?php echo sanitize_text_field( $value->comment_content ); ?></p>
	...
	function reset_flagged_reports( $comment_id ){ 
		...
		if( !empty($reports) ){ ?>
			...
			<span class="reset_flagged_reports"> | 
				<a href="javascript:void(0)" id="reset_flagged_reports" comment_id="<?php echo $comment_id; ?>"><?php _e( 'Reset Flagged Reports' , 'wpad' ); ?></a>
			</span>
	...
	function register_wp_advance_comment() {
		...
		add_menu_page( 'WP Advanced Comment', __( 'WP Advanced Comment' , 'wpad' ), 'delete_others_pages', 'wp_advance_comment', array(&$this, 'backend_settings') , 'dashicons-testimonial' , 25 );
